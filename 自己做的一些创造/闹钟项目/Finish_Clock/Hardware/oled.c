...
void IIC_Start(void)
{
	...
	Delay_us(6);
	IIC_SCL = 0; // 拉低 I2C 总线，准备发数据
}	

/*******************************************************************************
* Function Name: IIC_Stop
* Description: 产生 IIC 停止信号
*******************************************************************************/
void IIC_Stop(void)
{
	SDA_OUT();
...
* Description: IIC 读取一个字节
* Parameters: ack=1 时发送 ACK，ack=0 发送 NACK
* Return: 读到的数据
*******************************************************************************/  
...
{      			    
	u8 temp, t, t1;
	u8 y0 = y;
	u8 csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size / 2); // 计算一个字符占用的字节数
	chr = chr - ' '; // 得到偏移后的索引
    for(t = 0; t < csize; t++)
    {   
		if(size == 12) temp = ascii_1206[chr][t];     // 1206 字体
		else if(size == 16) temp = ascii_1608[chr][t]; // 1608 字体
		else if(size == 24) temp = ascii_2412[chr][t]; // 2412 字体
		else return; // 没这种字体就直接返回
        for(t1 = 0; t1 < 8; t1++)
		{
...
{	  	
	...
	OLED_WR_Byte(0xAE, OLED_CMD); // 先关显示
	...
	OLED_WR_Byte(0x02, OLED_CMD); // [1:0],00 行地址;01 列地址;10 页地址;默认 10
	OLED_WR_Byte(0xA1, OLED_CMD); // 段重定义设置,bit0:0->0;1->127
	OLED_WR_Byte(0xC0, OLED_CMD); // 设置 COM 扫描方向;bit3:0 正常;1 反向
	OLED_WR_Byte(0xDA, OLED_CMD); // 设置 COM 硬件引脚配置
	OLED_WR_Byte(0x12, OLED_CMD); // [5:4] 配置
		 
	OLED_WR_Byte(0x81, OLED_CMD); // 对比度设置
	OLED_WR_Byte(0xEF, OLED_CMD); // 亮度 1~255; 默认 0X7F
	OLED_WR_Byte(0xD9, OLED_CMD); // 设置预充电周期
	OLED_WR_Byte(0xf1, OLED_CMD); // [3:0] PHASE1; [7:4] PHASE2
	OLED_WR_Byte(0xDB, OLED_CMD); // 设置 VCOMH 电压倍率
	OLED_WR_Byte(0x30, OLED_CMD); // [6:4] 000=0.65*vcc;001=0.77*vcc;011=0.83*vcc

	OLED_WR_Byte(0xA4, OLED_CMD); // 全局显示开启;bit0:1 开启;0 关闭(白/黑屏)
	OLED_WR_Byte(0xA6, OLED_CMD); // 设置显示方式;bit0:1 反相;0 正常
...
{
    // 默认用 16 点阵字符：宽 8 像素、高 16 像素
    *x = (column - 1) * 8;   // 列 1~16 -> X 像素 0~127
    *y = (line   - 1) * 16;  // 行 1~4  -> Y 像素 0~63
}

// 行/列版显示字符串：接口类似 OLED_ShowString(1,1,"xxx");
...
// 行/列版显示数字：接口类似 OLED_ShowNum(1,6,num,4);
void OLED_ShowNum_RC(u8 line, u8 column, u32 num, u8 len)
{
    u8 x, y;
    RC_To_XY(line, column, &x, &y);
    OLED_ShowNum(x, y, num, len, 16);            // 浣跨敤 16 鐐归樀瀛椾綋
}
